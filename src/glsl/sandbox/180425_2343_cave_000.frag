...
#define snd sound()
#define XLIGHT 2.
#define XSPEED (1./snd) // (1.0/snd)
#define XGLOW  (1./snd) // (1.0/snd)
#define XSPACE 0. // bool
...
float voronoi(vec3 x) {
	...
	return res.y - res.x * (snd*XGLOW);
}

vec2 path(float z) {
	return vec2(cos(z/8.0)*sin(z/12.0)*12.0/(snd*XSPEED), 0.0);
}
...
float map(vec3 p) {
	...
	vec2 tun = abs(p.xy - path(p.z))*vec2(0.6-snd*XSPACE, 0.5);
