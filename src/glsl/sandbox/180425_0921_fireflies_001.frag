...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
  ...
  vec3 rd = cam * normalize(vec3(uv,.5 + snd));
  float dist = raymarch(ro, rd);
  float fog = 1.5 * dist * .1 / (100.0 + dist * dist * 0.00001);
  vec2 uw = fragCoord / iResolution.xy;
  fragColor = vec4(fog * vec3(uw.y, uw.x, 0.5),1.0);
}
